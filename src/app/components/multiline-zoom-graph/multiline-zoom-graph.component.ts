...
export class MultilineZoomGraphComponent implements AfterViewInit, OnChanges {
  ...
  ngOnChanges() {   
    ...
    let colorObj = ["#CF62A8", "#B072CC", "#D4772C", "#7986D8", "#E7A500"]
    this.allLines = [];
  ...
  private drawHover() {
    ...
    this.svg
      ...
      .on("mouseover", () => {
        ...
        const processedKey = key.split(" ").map(e => e.toLowerCase()).join("-");
    ...
    function mousemove() {
      try {
        ...
        self.legendHover.map(function (legend) {
          for (let i = 0; i < self.graphLinesData.length; i++) {
        const processedKey = self.graphLinesData[i].key.split(" ").map(e => e.toLowerCase()).join("-");
            ...
            for (let j = 0; j < currentLineValues.length; j++) {
              ...
              if (valueDate.toString() === hoverDate.toString()) {
                dobj["value-" + processedKey] = currentLineValues[j].value;
                break;
              }
            }
          }
        ...
        const processedKey = self.multiSelectForm.value[m].split(" ").map(e => e.toLowerCase()).join("-");   
